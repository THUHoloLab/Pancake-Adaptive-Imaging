...
def parse_args():
    ...
    parser.add_argument("--dataroot_gt", type=Path, default=Path("data/Datapancake1808-1017/train_ground_truth"))
    parser.add_argument("--dataroot_lq", type=Path, default=Path("data/Datapancake1808-1017/train_capture"))
    parser.add_argument("--dataroot_gt_val", type=Path, default=Path("data/Datapancake1808-1017/valid_ground_truth"))
    parser.add_argument("--dataroot_lq_val", type=Path, default=Path("data/Datapancake1808-1017/valid_capture"))
...
def main():
    ...
    # Dataset resolution (train/val nodes if present)
    ds_cfg = yaml_cfg.get("datasets", {})
    train_ds = ds_cfg.get("train", {})
    val_ds = ds_cfg.get("val_1", {})
    # Resolve dataset roots (train for training loop; val_1 for validation loop if present)
    ...
    # Run an initial validation using the pretrained weights if available
